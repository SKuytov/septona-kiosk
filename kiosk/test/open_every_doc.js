...
(async () => {
  ...
  const ctx = await browser.newContext({ viewport: { width: 1080, height: 1920 }, locale: 'bg-BG' });
  ...
  for (let ti = 0; ti < tabCount; ti++) {
    ...
    for (let ci = 0; ci < cards; ci++) {
      ...
      if (verdict === 'ok') { ok++; process.stdout.write('.'); if (ci % 5 === 4) console.log(''); }
      ...
      if (await page.locator('canvas').first().isVisible().catch(() => false)) {
        /*
          The close button is the last one in the document's own top bar. This used to be
          `.vw__close, .vbtn` — `.vw__close` no longer exists, so the fallback picked the
          first `.vbtn` on the page, which is the disabled previous-page button. Playwright
          then waited its full timeout for that button to become clickable on every single
          document, which is why this sweep appeared to hang.
        */
        await page
          .locator('.vw__bar .vbtn')
          .last()
          .click({ timeout: 4000 })
          .catch(() => {});
